...
- Keep the experimental `lighter` profile strictly isolated to `LUTKW`, which
  ...
  phase-7 selector changes from `0xA8` to `0xAA`, replacing its fourth
  two-frame GND selector with VDL while preserving Seeed's first three VDL
  selectors. The real-device test of 0.3.55 proved that the former `0x28`
  experiment made the filled light-gray cards darker, so never restore that
  profile as a lightening change.
  ...
  the revised lighter waveform is proven on real hardware.
...
- `guesty_render_revision` invalidates otherwise-identical images after a
  ...
  and update their tests. The current source revision is 37.
