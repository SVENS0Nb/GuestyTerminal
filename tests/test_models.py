...
def test_payload_expires_at_checkout() -> None:
    options = MappingOptions(
        endpoint_entity="sensor.guestyterminal_display_1_guesty_terminal_endpoint",
        listing_id="listing-1",
    )
    ...
    assert not payload.is_expired(datetime(2026, 8, 17, 8, 59, tzinfo=UTC))
    assert payload.is_expired(datetime(2026, 8, 17, 9, 0, tzinfo=UTC))
